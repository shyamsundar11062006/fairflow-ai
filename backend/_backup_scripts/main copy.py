...
def get_driver_history(user_email: str, db: Session = Depends(get_db)):
    ...
    # Query fairness_snapshots (actual data) instead of fairness_history
    snapshots = db.query(models.FairnessSnapshot).filter(
        models.FairnessSnapshot.driver_id == user.id
    ).order_by(models.FairnessSnapshot.date.desc()).limit(7).all()
    
    # Convert snapshots to history display format
    history = []
    for snapshot in snapshots:
        history.append({
            "date": snapshot.date,
            "daily_effort": snapshot.effort,
            "team_average": 60.0,  # Default baseline (can be calculated if needed)
            "credits_earned": snapshot.credits,
...
def get_admin_dashboard(db: Session = Depends(get_db)):
    ...
    for driver in drivers:
        ...
        # Calculate team average for adaptive threshold
        team_avg = logic.calculate_team_average(db)
        
        # Determine Alert Logic based on adaptive threshold
        alert_msg = None
        reason = None
        
        # Check if driver is absent
        if driver.status == "ABSENT":
            alert_msg = "Absent"
            reason = "Driver is not available today"
        # Check if driver is sick
        elif driver.status == "SICK":
            alert_msg = "Protected"
            reason = "Sick driver protected from hard routes"
        # Check balance safeguard
        elif stats.total_balance < -25:
            alert_msg = "Protected"
            reason = f"Balance ({stats.total_balance}) below threshold (-25)"
        # Check if underloaded
        elif stats.total_balance > 25:
            alert_msg = "Underloaded"
            reason = "High positive balance, ready for hard routes"

        # 7-Day Stats Logic
        stats_7d = logic.get_driver_stats_7d(db, driver.id)
        readiness = logic.get_readiness_status(stats_7d["balance_7d"])
...
def auto_assign_route(request: schemas.BatchRecommendationRequest, db: Session = Depends(get_db)):
    ...
    if not best_options:
        # Fallback: If everyone is assigned, allow re-assignment or error out
        # For prototype, let's pick the absolute best rank even if already assigned
        # but we prioritize the truly available ones first.
        best_overall = [r for r in ranked_results if r['preference_rank'] is not None]
        best_overall.sort(key=lambda x: x['preference_rank'])
        if not best_overall:
            raise HTTPException(status_code=400, detail="No eligible drivers found for assignment")
        best_fit = best_overall[0]
    else:
...
def get_ml_route_recommendation(
    ...
    # Get normalized effort (per working hour) for last 3 days
    effort_dist = get_effort_distribution(db, last_n_days=3)
    driver_normalized_effort = effort_dist.get(driver_id, 7.5)
...
def get_ml_route_recommendations_batch(
    ...
    # 2. Determine objective route difficulty for preference weighting
    # We use recommender's internal score here because it matches the MockRoute structure
    obj_score = recommender._calculate_route_score(route_features)
    if obj_score < 45:
        route_difficulty = "Easy"
    elif obj_score <= 65:
